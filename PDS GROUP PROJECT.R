...
rm(list=ls())
dataog <- read.csv(file = "D:/RKMVERI MSc BDA/PDS project/Vaccination.csv",header = TRUE)
head(dataog)
...
bar <- ggplot(data=df,aes(y=Freq,x=Var1),beside=T)+geom_bar(stat="identity",width = 0.5,col="blue",fill="red")+
...
bar1 <- ggplot (df1, aes(y = Freq,x = "", fill= Var1))+geom_bar(stat = "identity", width = 0.5)+
  labs(title = "Gender Composition", x= "Gender", y= "Frequency",fill="Gender")
pie<- bar1+ coord_polar("y", start = 0)
...
bar <- ggplot(data=df2,aes(y= Freq,x= Var1),beside=T)+geom_bar(stat="identity",width = 0.3,col="blue",fill="red")+
  labs(title="Living area",x="Resident area type",y="Frequency",
       subtitle = "1 represents Metropolitan city
2 represents Rural
3 represents Small town")
bar
...
bar1 <- ggplot(data=df2,aes(y=Freq ,x="",fill= Var1))+geom_bar(stat="identity",width = 0.5)+
  ...
  '2' Represents Rural area.
  '3' Represents Small Town.",y="Resident area type",fill="Resident area type")
pie <- bar1 + coord_polar("y",start=0)
...
ggplot(data=demo4,aes(x=Age,fill=Living_area,y=freq_density))+
  geom_bar(position="dodge",stat="identity",width =0.5)+facet_wrap(~Gender)+
  labs(y="Frequency density",x="Age intervals",title="Gender wise Living area data",
...
ggplot(data=age_safe,aes(x=Age,y=freq_den,fill=as.factor(Vac_safe)),beside=TRUE)+geom_bar(position="dodge",stat="identity",width=0.3)+
  labs(title="Reliability on Vaccination",subtitle="  1 represents No, it can affect my health 
  2 represents Not Sure 
  3 represents Yes",x="Age intervals",y="Frequency density",fill="Reliability on vaccine")
...
waiting <- travel_no %>% 
  group_by(waiting_time,Living_area) %>% 
  summarise(count= n())
